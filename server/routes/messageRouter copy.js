...
router.post('/upload', (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
    if (err) return res.send(response.fail(`文件上传失败${err}`))
    
    console.log(files.hash, files.filename, files.chunk, fields, 'filessss')
    let uploadPath = files.file // 文件
    console.log(uploadPath, 'uploadPathuploadPath')
    if(!uploadPath) return res.send(response.fail('没有读取到文件, 请重新调试'))
